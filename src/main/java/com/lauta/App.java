...
/**
 * This is the main entry point for this application. This will prompt the user
 * for three sides, construct the appropriate triangle and print its name.
 *
 * @author Dave
 */
public class App {
    ...
    private double[] parseInput(String[] args) {
        ...
        if (args.length >= 3) {
            try {
                ...
                System.out.println("Invalid parameters to the program. " + e.getClass().getName() + " " + e.getMessage());
            }
        }
        Scanner scan = new Scanner(System.in);
        try {
            System.out.print("Please enter a value for the first side's length: ");
            sides[index++] = (scan.nextDouble());
            System.out.print("Please enter a value for the second side's length: ");
            sides[index++] = (scan.nextDouble());
            System.out.print("Please enter a value for the third side's length: ");
            sides[index++] = (scan.nextDouble());
        } catch (Exception e) {
            System.out.println("Invalid parameters to the program. " + e.getClass().getName() + " " + e.getMessage());
            sides[0] = 0.0;
